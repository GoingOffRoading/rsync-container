...
# Source directory to sync from, configurable via ENV
SRC_DIR="${SRC_DIR:-/host/}"
...
echo "Target directory found. Starting rsync from ${SRC_DIR} to ${USB_TARGET_DIR}"
echo "Timestamp: $(date)"
echo "Using command: ${RSYNC_CMD} ${SRC_DIR} ${USB_TARGET_DIR}"

${RSYNC_CMD} "${SRC_DIR}" "${USB_TARGET_DIR}"
